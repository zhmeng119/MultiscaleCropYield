...
a <- c(rawz_whole %>% filter(.,time %>% substring(.,1,4)==2018) %>% 
         select(.,NDVI)) %>% unlist(use.names=FALSE)
...
# y <- p_dat_680_d$NDVI
y <- c(p_dat_680_d %>% filter(.,time %>% substring(.,1,4)==2018) %>% 
         select(.,NDVI) %>% unlist(use.names=FALSE))

x <- c(p_dat_680_d %>% filter(.,time %>% substring(.,1,4)==2018) %>% 
         select(.,SWdw) %>% unlist(use.names=FALSE))
# x <- c(1:length(y))
...
legend(0,1, 
       legend = paste(c("1st", "2nd", "3rd", "4th"), "degree"),
